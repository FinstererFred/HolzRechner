...
ALTER TABLE `staemme`
  CHANGE COLUMN `kubatur` `kubatur_mit` FLOAT NULL DEFAULT NULL AFTER `staerkeklasse`,
  ADD COLUMN `kubatur_ohne` FLOAT NULL DEFAULT NULL AFTER `kubatur_mit`,
  DROP COLUMN `rindenabzug`;

  ALTER TABLE `staemme`
  CHANGE COLUMN `kubatur_mit` `kubatur_mit` DECIMAL(10,2) NULL DEFAULT NULL AFTER `staerkeklasse`,
  CHANGE COLUMN `kubatur_ohne` `kubatur_ohne` DECIMAL(10,2) NULL DEFAULT NULL AFTER `kubatur_mit`;
